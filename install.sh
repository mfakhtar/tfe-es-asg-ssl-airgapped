...
REPLICATED_VERSION="2.55.0"
PINNED_DOCKER_VERSION="20.10.17"
...
pullReplicatedImages() {
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ]; then
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.55.0"
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.55.0"
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated:stable-2.55.0" "${REPLICATED_REGISTRY_PREFIX}/replicated:stable-2.55.0")
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-ui:stable-2.55.0" "${REPLICATED_REGISTRY_PREFIX}/replicated-ui:stable-2.55.0")
    else
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated:stable-2.55.0"
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated-ui:stable-2.55.0"
    fi
}
...
pullOperatorImage() {
    if [ -n "$REGISTRY_ADDRESS_OVERRIDE" ]; then
        docker pull "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.55.0"
        (set -x; docker tag "${REGISTRY_ADDRESS_OVERRIDE}/${REGISTRY_PATH_PREFIX}replicated/replicated-operator:2.55.0" "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.55.0")
    else
        docker pull "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.55.0"
    fi
}
...
tagAndPushOperatorImage()  {
    docker tag \
        "${REPLICATED_REGISTRY_PREFIX}/replicated-operator:2.55.0" \
        "${1}/replicated/replicated-operator:2.55.0"
    docker push "${1}/replicated/replicated-operator:2.55.0"
}
...
tag_docker_images() {
    ...
    docker tag "$REPLICATED_REGISTRY_PREFIX/replicated:stable-2.55.0" "$REPLICATED_REGISTRY_PREFIX/replicated:current" \
        || docker tag -f "$REPLICATED_REGISTRY_PREFIX/replicated:stable-2.55.0" "$REPLICATED_REGISTRY_PREFIX/replicated:current"
    docker tag "$REPLICATED_REGISTRY_PREFIX/replicated-ui:stable-2.55.0" "$REPLICATED_REGISTRY_PREFIX/replicated-ui:current" \
        || docker tag -f "$REPLICATED_REGISTRY_PREFIX/replicated-ui:stable-2.55.0" "$REPLICATED_REGISTRY_PREFIX/replicated-ui:current"
}
...
install_operator() {
    ...
    if [ "$AIRGAP" != "1" ]; then
        ...
        echo -e "${URLGET_CMD} https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.55.0${NC}"
        ${URLGET_CMD} "https://get.replicated.com${prefix}/operator?replicated_operator_tag=2.55.0" > "$REPLICATED_TEMP_DIR/operator_install.sh"
    fi
